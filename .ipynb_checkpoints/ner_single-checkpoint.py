...
def predict_ner(sentence):
    ...
    parser.add_argument("--model_name_or_path", default="transformers/bert_models/", type=str, required=False,
                        help="Path to pre-trained model or shortcut name selected in the list: " + ", ".join(
                            ALL_MODELS))
    parser.add_argument("--output_dir", default="output_models/PL-Marker-42/", type=str, required=False,
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result = evaluate_ner(sentence, args, model, tokenizer, prefix=global_step, do_test=not args.no_test)
