...
class ACEDataset_re_single(Dataset):
    def __init__(self, sentence,  tokenizer, args=None, evaluate=False, do_test=False, max_pair_length=None):
        ...
        if args.data_dir.find('ace05') != -1:
            ...
            self.ner_label_list = ['NIL', 'domain', 'subject', 'object']

            if args.no_sym:
                label_list = ['主营','产品','细分主营','研究领域','应用领域','合作客户','上游企业','下游企业']
                self.sym_labels = ['NIL']
                self.label_list = self.sym_labels + label_list
...
def predict(sentence):
    ...
    parser.add_argument("--model_name_or_path", default="transformers/bert_models/", type=str, required=False,
                        help="Path to pre-trained model or shortcut name selected in the list: " + ", ".join(
                            ALL_MODELS))
    parser.add_argument("--output_dir", default="output_models/re-bert-42", type=str, required=False,
    ...
    parser.add_argument('--max_pair_length', type=int, default=16, help="")
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result = evaluate(sentence, args, model, tokenizer, prefix=global_step, do_test=not args.no_test)
...
if __name__ == "__main__":
    ...
    sentence = "新华龙:主要从事钼炉料、钼化工、钼金属等钼系列产品的生产、加工、销售业务"
    sentence = list(sentence)
    input = {"clusters": [[], []], "sentences": [sentence
       ], "ner": [[], []], "relations": [[], []], "doc_key": "J87-1003"}

    input_json = json.dumps(input)
    pred_ner = predict_ner(input_json)


    pred_ner = [list(ele) for ele in pred_ner[0]]


    input['predicted_ner'] = [pred_ner]
    input_json = json.dumps(input)
